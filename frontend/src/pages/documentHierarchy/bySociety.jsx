...
function bySociety() {
  ...
  const getUserList = useCallback(() => {
    getAPI(`/reports/society?skip=0&limit=0`)
      .then((res) => {
        if (res?.data?.status) {
  ...
  const transformCompanyList = (companies) => {
    ...
    companies.forEach((company) => {
      ...
      // SFP rows
      const sfpList = company?.sfp || [];
      sfpList.forEach((item) => {
        rows.push({
          type: "sfp",
  ...
  const rawData = transformCompanyList(societyList);
  ...
  return (
    <>
      <div className="d-flex justify-content-between p-2 bg-white">
        <div className="p-2">
          <h6 className="fw-bold mb-0">Document Hierarchy By Society</h6>
        </div>
      </div>
