...
const schema = yup.object().shape({
  schoolNo: yup.string().required("School No is required"),
  ...
  return (
    <div className="card p-4 pt-2 shadow">
      <div className="d-flex align-items-center justify-content-between mb-3 border-bottom">
        ...
        <h6 className="fw-bold text-dark mb-0">School 1 - {location.state?.name}</h6>
        <div />
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="row">
          <div className="col-md-4 mb-3">
            <label className="form-label">School No</label>
            <input
              {...register("schoolNo")}
              className={`form-control ${errors.schoolNo ? "is-invalid" : ""}`}
              placeholder="Enter School No"
            />
            <div className="invalid-feedback">{errors.schoolNo?.message}</div>
          </div>
          <div className="col-md-4 mb-3">
