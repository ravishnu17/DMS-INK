...
const schema = yup.object().shape({
  boardingHostelNo: yup.string().required("Boarding & Hostel No is required"),
      ...
      if (isDirty) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            navigate("/nonfinancial/boardingHostel");
          }
        });
      } else {
        navigate("/nonfinancial/boardingHostel");
  ...
  return (
      <div className="card p-4 pt-2 shadow">
        <div className='d-flex align-items-center justify-content-between mb-3 border-bottom'>
          ...
          <h6 className="fw-bold text-dark mb-0">Boarding & Hostel 1 - {location.state?.name}</h6>
          <div />
        </div>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="row">
            <div className="col-md-4 mb-3">
              <label className="form-label">Boarding & Hostel No</label>
              <input {...register("BoardingHostelNo")} className={`form-control ${errors.boardingHostelNo ? "is-invalid" : ""}`} placeholder="Enter Boarding & Hostel No" />
              <div className="invalid-feedback">{errors.boardingHostelNo?.message}</div>
            </div>
            <div className="col-md-4 mb-3">
...
export default AddBoardingHostel;
