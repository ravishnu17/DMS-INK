...
import { departmentRoutes, docsRoute } from '../../routes';
...
function DepartmentView() {
  ...
  const data = [
    { id: 1, name: "Monthly Accounts Abstract", type: 'Annual', date: formatDate(new Date()) },
    { id: 2, name: "Bank Signature Change Confirmation", type: 'Annual', date: formatDate(new Date()) },
    { id: 3, name: "Annual Budget", type: 'Annual', date: formatDate(new Date()) },
  ];
  const columns = [
            ...
            <div className="d-flex justify-content-between">
              <div className="form_col ml-1">
                <span className="custum-group-table">
                  <button type="button" className="btn  btn-sm text-info" onClick={() => { navigate('/nonfinancial/department/departmentList', { state: { name: row.name } }) }} title='List'>
              ...
              <div className="form_col ml-1">
                <span className="custum-group-table">
                  <button type="button" className="btn  btn-sm text-success" title='Add' onClick={() => { navigate("/nonfinancial/department/departmentAdd", { state: { name: row.name } }) }}>
              ...
              <div className="form_col ml-1">
                <span className="custum-group-table">
                  <button type="button" className="btn  btn-sm text-primary" title='Documents' onClick={() => { navigate("/nonfinancial/department/docsview", { state: { name: row.name } }) }}>
      ...
      <div >
        <div className="d-flex justify-content-between align-items-center p-2 bg-white">
          <div class="d-flex align-items-center gap-2">
            <button className='btn pb-0' type='button' onClick={() => navigate('/department')}>
...
function Department() {
  return (
    <Suspense>
      <Routes>
        {[{ path: '/', element: DepartmentView }, ...departmentRoutes, ...docsRoute].map((route, index) => (
          route.element && <Route
            key={index}
            path={route.path}
