...
const schema = yup.object().shape({
  socialSectorNo: yup.string().required("Social Sector No is required"),
      ...
      if (isDirty) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            navigate("/nonfinancial/socialSector");
          }
        });
      } else {
        navigate("/nonfinancial/socialSector");
  ...
  return (
      <div className="card p-4 pt-2 shadow m-2">
        <div className='d-flex align-items-center justify-content-between mb-3 border-bottom'>
          ...
          <h6 className="fw-bold text-dark mb-0">Social Sector 1 - {location.state?.name}</h6>
          <div />
        </div>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="row">
            <div className="col-md-4 mb-3">
              <label className="form-label">Social Sector No</label>
              <input {...register("socialSectorNo")} className={`form-control ${errors.socialSectorNo ? "is-invalid" : ""}`} placeholder="Enter Social Sector No" />
              <div className="invalid-feedback">{errors.socialSectorNo?.message}</div>
            </div>
            <div className="col-md-4 mb-3">
...
export default SocialSectorAdd;
