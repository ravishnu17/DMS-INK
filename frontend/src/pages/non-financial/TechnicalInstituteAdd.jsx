...
const schema = yup.object().shape({
  technicalInstituteNo: yup.string().required("Technical Institute No is required"),
...
function TechnicalInstituteAdd() {
    ...
    if (isDirty) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          navigate("/nonfinancial/technicalInstitute");
        }
      });
    } else {
      navigate("/nonfinancial/technicalInstitute");
  ...
  return (
    <div className="card p-4 pt-2 shadow">
      <div className="d-flex align-items-center justify-content-between mb-3 border-bottom">
        ...
        <h6 className="fw-bold text-dark mb-0">Technical Institute 1 - {location.state?.name}</h6>
        <div />
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="row">
          <div className="col-md-4 mb-3">
            <label className="form-label">Technical Institute No</label>
            <input
              {...register("technicalInstituteNo")}
              className={`form-control ${errors.technicalInstituteNo ? "is-invalid" : ""}`}
              placeholder="Enter Technical Institute No"
            />
            <div className="invalid-feedback">{errors.technicalInstituteNo?.message}</div>
          </div>
          <div className="col-md-4 mb-3">
...
export default TechnicalInstituteAdd;