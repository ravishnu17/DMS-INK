...
function Parish() {
  ...
  //   diocese_id: yup.object().required("Diocece is required"),
  ...
  const [parishcode, setParishCode] = useState("PAR0007");
  ...
  // console.log("financialPortfolios", financialPortfolios);

  // const currentUser = contextProp?.currUser;
  // const permissions = contextProp?.permissions;
  // const modulepermission = permissions?.role_permissions?.parishes;
  ...
  useEffect(() => {
    ...
    if (contextProp?.permissions) {
      setmodulepermission(
        contextProp?.permissions?.role_permissions?.parishes || {}
      );
    }
  }, [contextProp]);
  ...
  const dioceseOptions = dioceseList.map((comm) => ({
    value: comm.id,
    label: comm.name,
  }));
  ...
  const getDioceseList = () => {
    getAPI("/config/diocese-options?skip=0&limit=0")
      .then((res) => {
        if (res?.status) {
          setDioceseList(res?.data?.data);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const getPortfolio = () => {
    setLoading(true);
    getAPI(
      "/config/portfolio?skip=0&limit=25&type=Non%20Financial&search=Parish"
    )
      .then((res) => {
        if (res?.data?.status) {
  ...
  const getPortFolioList = useCallback(
    (search) => {
      ...
      if (portfolioIds != undefined && search != undefined) {
        getAPI(
          `/config/entity?skip=${pagination?.skip}&limit=${pagination?.limit}&portfolio_id=` +
            portfolioIds +
            "&search=" +
            search
        )
      ...
      .then((res) => {
        if (res?.data?.status) {
          if (type === "expanded") {
            ...
            setValue("diocese_id", {
              value: res?.data?.data?.diocese?.id,
              label: res?.data?.data?.diocese?.name,
            });
                  ...
                  {/* <div className="form_col ml-1">
                      <span className="custum-group-table" >
                        <button type="button" className="btn  btn-sm text-info" title='Manage files'
                          //  onClick={() => { contextProp.setNavState({ ...row, module: 'Parish' }); navigate('/nonfinancial/parish') }}
                          onClick={() => mainManageFiles(row, navigate, portfolioIds, 'parish')}
                        >
                  ...
                  <div className="form_col ml-1">
                    <span className="custum-group-table">
                      <button
                        ...
                        onClick={() =>
                          mainManageFiles(row, navigate, portfolioIds, "parish")
                        }
                      >
                        <i
  ...
  const onSubmit = (data) => {
    ...
    let apiData = {
      diocese_id: Number(data?.diocese_id?.value) || null,
      community_id: Number(data?.community_id?.value) || null,
  ...
  const onSubmit1 = (data) => {
    ...
    const mapping = "POST";
    addUpdateAPI(mapping, "config/parishes/import", formData).then((res) => {
      if (res?.data?.status) {
        getPortFolioList();
        getUserOptions();
        setLoading(false);
  ...
  const downloadExcel = () => {
    fetch(API_BASE_URL + "config/entities/sample-export?portfolio_id=3", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = "Parish_Form.xlsx"; // Change the file name if needed
  ...
  const downloadCommunityExcel = () => {
    fetch(API_BASE_URL + "config/entity-portfolio/export?portfolio_id=3", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((response) => {
        response.blob().then((blob) => {
          ...
          const a = document.createElement("a");
          a.href = url;
          a.download = "Parish_List.xlsx"; // Change the file name if needed
          document.body.appendChild(a);
          a.click();
          a.remove();
        });
    ...
    <>
      <div>
        <div className="d-flex justify-content-between p-2 flex-wrap bg-white ">
          ...
          <div className="d-flex justify-content-end col-lg-7 p-2 col-12 flex-wrap gap-2 align-items-end">
            ...
            <button className='btn btn-sm    btn-success' data-bs-toggle="modal" data-bs-target="#fileImportModal" title='Import' onClick={() => {
            ...
            <button className='btn btn-sm   btn-primary' title='Export' onClick={downloadCommunityExcel} >Export </button> */}
            {currentUser?.role?.name === "Admin" ||
            currentUser?.role?.name === "Super Admin" ? (
              <>
                <button
                ...
                <div className="row ms-1">
                  <div className="mb-1 col-md-3">
                    <label className="form-label">
                      Diocese <span className="text-danger">*</span>
                    </label>
                    <Controller
                      name="diocese_id"
                      control={control}
                    ...
                    />
                    <p className="text-danger">{errors.diocese_id?.message}</p>
                  </div>
                  <div className="mb-1 col-md-3">
                    <label className="form-label">
                  ...
                  {/* <div className="col-md-3 mb-1">
                    ...
                    <input
                      ...
                      {...register("code", {
                        required: "Parish code is required",
                      })}
                    />
      ...
      >
        <div className="modal-dialog modal-xl">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">View Parish</h5>
              <button
                type="button"
                className="btn-sm btn-close"
                data-bs-dismiss="modal"
            ...
            <div className="modal-body">
              <div className="row ms-1 border-bottom border-secondary mb-5">
                ...
                <div className="col-md-3 mb-1">
                  <label className="form-label">Parish Code</label>
                ...
                {/* <div className="col-md-3 mb-1">
                  ...
                  <p className="ms-2 fw-bold">{watch("accountant")}</p>
                </div> */}
      ...
      >
        <div className="modal-dialog modal-md">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Import Parish</h5>
              <button
                type="button"
                className="btn-sm btn-close"
                data-bs-dismiss="modal"
