...
function SocialSector() {
  ...
  // const modulepermission = permissions?.role_permissions?.[`social sectors`];
  ...
  useEffect(() => {
    ...
    if (contextProp?.permissions) {
      setmodulepermission(
        contextProp?.permissions?.role_permissions?.[`social sectors`] || {}
      );
    }
  }, [contextProp]);
  ...
  const [socialSectorList, setSocialSectorList] = useState([]);
  ...
  const initial = {
    ...
    // Initialize dynamic modules with empty values
    // ...dynamicModules.reduce((acc, module) => {
    //   return {
    //     ...acc,
    //     [`portfolio_${module.portfolio_id}`]: {
    //       [module.type]: null,
    //       [module.key]: "",
    //       [module.key1]: "",
    //       incharge_user_id: null,
  ...
  //   getSocialSectorList(e.target.value)
  // };

  const handleSearch = (e) => {
    getPortfolios(e.target.value);
  };
  ...
  const getPortfolio = () => {
    getAPI(
      "/config/portfolio?skip=0&limit=25&type=Non%20Financial&search=Social Sector"
    )
      .then((res) => {
        if (res?.data?.status) {
  ...
  const getPortfolios = useCallback(
    (search) => {
      if (portfolioIds) {
        setLoading(true);
        getAPI(
          `/config/entity?skip=${pagination?.skip}&limit=${
            pagination?.limit
          }&portfolio_id=${portfolioIds}&search=${search ? search : ""}`
        )
          .then((res) => {
            if (res?.data?.status) {
  ...
  const createViewDynamicModules = (backendData) => {
    const modules = backendData.map((item, index) => {
      ...
      return {
        ...
        [`no`]: number,
        [`name`]: item?.name || "N/A",
        [`type`]: item?.type || "N/A", // Ensure type is included
      ...
      .then((res) => {
        if (res?.data?.status) {
          if (type === "expanded") {
            ...
            setValue("ug_pg", res?.data?.data?.ug_pg);
            setValue("affiliation", res?.data?.data?.affiliation);
            setValue("faculty", res?.data?.data?.faculty);
            setValue(
              "financialAssistance",
  ...
  const rowExpandView = (row) => {
    return (
      <div className="p-2 ps-3 pt-3 subtable d-flex ">
        <table className="table bg-light rounded overflow-hidden w-75">
          <thead>
  ...
  const onSubmit = (data) => {
    ...
    const url = !isEdit ? "/config/entity/" : `/config/entity/${data?.id}/`;
  ...
  const columns = [
    ...
    { name: "Place", selector: (row) => row.place, width: "150px" },
    {
      name: "Incharge",
      width: "200px",
      cell: (row, index) => {
                  ...
                  {/* <div className="form_col ml-1">
                      <span className="custum-group-table" >
                        <button type="button" className="btn  btn-sm text-info" title='Manage files'
                          // onClick={() => { contextProp.setNavState({ ...row, module: 'Social Sector' }); navigate('/nonfinancial/socialSector') }} 
                          onClick={() => mainManageFiles(row, navigate, portfolioIds, 'socialSector')}
                        >
                  ...
                  <div className="form_col ml-1">
                    <span className="custum-group-table">
                      <button
                        ...
                        onClick={() => {
                          getAuditTrail(row.id), getPortfolioDetails(row.id);
                        }}
                      >
  ...
  const onSubmit1 = (data) => {
    ...
    const mapping = "POST";
    addUpdateAPI(mapping, "config/social-sectors/import", formData).then(
      (res) => {
        if (res?.data?.status) {
          getPortfolios();
          getUserOptions();
  ...
  const downloadExcel = () => {
    fetch(API_BASE_URL + "config/entities/sample-export?portfolio_id=9", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = "Social_Sector_Form.xlsx"; // Change the file name if needed
  ...
  const downloadCommunityExcel = () => {
    fetch(API_BASE_URL + "config/entity-portfolio/export?portfolio_id=9", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((response) => {
        response.blob().then((blob) => {
          ...
          const a = document.createElement("a");
          a.href = url;
          a.download = "Social_Sector_List.xlsx"; // Change the file name if needed
          document.body.appendChild(a);
          a.click();
          a.remove();
        });
    ...
    <>
      <div>
        <div className="d-flex justify-content-between p-2 flex-wrap bg-white">
          <div className="p-2 col-lg-6 col-12">
            {/* <h6 className="fw-bold mb-0">Social Sector</h6>
             */}
          ...
          <div className="d-flex justify-content-end col-lg-6 p-2 col-12 flex-wrap gap-2 align-items-end">
            <div className="d-flex flex-grow-1" style={{ maxWidth: "300px" }}>
              ...
              <input
                ...
                value={search}
                onChange={(e) => {
                  getPortfolios(e.target.value), setSearch(e.target.value);
                }}
              />
            </div>
            {/* {
              currentUser?.role?.name === "Admin" || currentUser?.role?.name === "Super Admin" ?
                ...
                <>  <button
                  className="btn btn-sm adminBtn px-4"
                  data-bs-toggle="modal"
                  data-bs-target="#addModel"
                  onClick={() => {
                    setIsEdit(false);
            ...
            currentUser?.role?.name === "Super Admin" ? (
              <>
                <button
                  ...
                  data-bs-target="#addModel"
                  onClick={() => {
                    setIsEdit(false), setSelectedData(null);
                    reset(initial);
                  }}
                >
                  Add{" "}
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">
                {isEdit ? "Edit Social Sector" : "Add Social Sector"}{" "}
              </h5>
              <button
                type="button"
                ...
                <div className="row ms-1">
                  ...
                  {/* <div className="mb-3 col-md-3">
                    ...
                    <input
                      ...
                      placeholder="Enter Social Sector Code"
                      {...register("code")}
                    />
                  ...
                  <div className="col-md-3 mb-1">
                    <label className="form-label">
                      Social Sector Name <span className="text-danger">*</span>
                    </label>
                    <input
                      type="text"
                      className="form-control"
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="row ms-1 border-bottom border-secondary mb-0">
                ...
                <div className="mb-1 col-md-3">
                  <label className="form-label">Audit-Reference Code</label>
                    ...
                    {Array.isArray(item?.viewer) && item.viewer.length > 0 ? (
                      <div className="ms-2 fw-bold mb-0 d-flex flex-wrap gap-1">
                        {item.viewer.map((viewer, idx) => (
                          <p key={idx} className=" fw-bold mb-0">
                            {viewer.name}
                          </p>
                        ))}
                      </div>
      ...
      >
        <div className="modal-dialog modal-md">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Import Social Sector</h5>
              <button
                type="button"
                className="btn-sm btn-close"
                data-bs-dismiss="modal"
