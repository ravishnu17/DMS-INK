...
function College() {
  ...
  // const modulepermission = permissions?.role_permissions?.colleges;
  ...
  useEffect(() => {
    ...
    if (contextProp?.permissions) {
      setmodulepermission(
        contextProp?.permissions?.role_permissions?.colleges || {}
      );
    }
  }, [contextProp]);
  ...
  //     financialAssistance: yup.string().required("Financial Assistance is required"),
  //     ug_pg: yup.string().required("UG/PG is required"),
  ...
  const buildValidationSchema = (modules) => {
    ...
    return yup.object().shape({
      ...
      ug_pg: yup.string().trim().required("UG/PG is required"),
  ...
  const getPortfolio = () => {
    setLoading(true);
    getAPI(
      "/config/portfolio?skip=0&limit=25&type=Non%20Financial&search=College"
    )
      .then((res) => {
        if (res?.data?.status) {
      ...
      .then((res) => {
        if (res?.data?.status) {
          if (type === "expanded") {
            ...
            // const viewerUsers = res?.data?.data?.entity_user.find(
            //   (user) => user.role_id === RoleIds.ViewerRoleId
            // );
  ...
  const onSubmit = (data) => {
    ...
    addUpdateAPI(mapping, url, submitdata).then((res) => {
      // console.log("Response from addUpdateAPI:", res);
      if (res?.data?.status) {
        // getPortFolioList(portfolioIds);
        setSearch("");
        getPortfolios();
        getUserOptions();
        setLoading(false);
  ...
  const columns = [
    ...
    { name: "Affiliation", selector: (row) => row?.affiliation },
    { name: "Faculty", selector: (row) => row?.faculty },
    { name: "UG/PG", selector: (row) => row?.ug_pg },
                  ...
                  {/* <div className="form_col ml-1">
                      <span className="custum-group-table" >
                        <button type="button" className="btn  btn-sm text-info" title='Manage files'
                          // onClick={() => { contextProp.setNavState({ ...row, module: 'College' }); navigate('/nonfinancial/college') }} 
                          onClick={() => mainManageFiles(row, navigate, portfolioIds, 'college')}
                        >
  ...
  const onSubmit1 = (data) => {
    ...
    const mapping = "POST";
    addUpdateAPI(mapping, "config/college/import", formData).then((res) => {
      if (res?.data?.status) {
        getPortfolios();
        getUserOptions();
        setLoading(false);
  ...
  const downloadExcel = () => {
    fetch(API_BASE_URL + "config/entities/sample-export?portfolio_id="+ String(collegePortfolioId), {
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = "College_Form.xlsx"; // Change the file name if needed
    ...
    fetch(API_BASE_URL + "config/entity-portfolio/export?portfolio_id="+ String(collegePortfolioId), {
      ...
      .then((response) => {
        response.blob().then((blob) => {
          ...
          const a = document.createElement("a");
          a.href = url;
          a.download = "College_List.xlsx"; // Change the file name if needed
          document.body.appendChild(a);
          a.click();
          a.remove();
        });
    ...
    <>
      <div>
        <div className="d-flex justify-content-between p-2 flex-wrap bg-white">
          <div className="p-2 col-lg-5 col-12">
            {/* <h6 className="fw-bold mb-0">College</h6> */}
                ...
                <div className="row ms-1">
                  ...
                  <div className="mb-1 col-md-3">
                    ...
                    >
                      ...
                      <option value="Tamil Nadu Teacher's Education University">
                        Tamil Nadu Teacher's Education University
                      </option>
                  ...
                  <div className="mb-1 col-md-3">
                    ...
                    >
                      ...
                      <option value="Arts and Science College">
                        Arts and Science College
                      </option>
                      <option value="Education College">
                        Education College
                      </option>
                      <option value="Agriculture College">
                        Agriculture College
                      </option>
                    </select>
                  ...
                  <div className="mb-1 col-md-3">
                    ...
                    <p className="text-danger">{errors.ug_pg?.message}</p>
                  </div>
                  <div className="mb-1 col-md-3">
                    <label className="form-label">
            ...
            <div className="modal-body">
              <div className="row ms-1 border-bottom border-secondary mb-5">
                ...
                <div className="mb-1 col-md-3">
                  ...
                  <p className="ms-2 fw-bold">
                    {watch("financialAssistance") || "N/A"}
                  </p>
                </div>
                <div className="mb-1 col-md-3">
                  <label className="form-label">Affiliation</label>
                  <p className="ms-2 fw-bold">
                    {watch("affiliation") || "N/A"}
                  </p>
                </div>
                <div className="mb-1 col-md-3">
                  <label className="form-label">Faculty</label>
                  <p className="ms-2 fw-bold">{watch("faculty") || "N/A"}</p>
                </div>
                <div className="mb-1 col-md-3">
                  <label className="form-label">UG / PG</label>
                  <p className="ms-2 fw-bold"> {watch("ug_pg") || "N/A"}</p>
                </div>
                <div className="mb-1 col-md-3">
      ...
      >
        <div className="modal-dialog modal-md">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Import College</h5>
              <button
                type="button"
                className="btn-sm btn-close"
                data-bs-dismiss="modal"
...
//   styleTag.innerHTML = stickyStyles;
