...
function Society() {
  ...
  // const modulepermission = permissions?.role_permissions?.society;

  //prevent re-rendering of currentUser and modulepermission
  ...
  useEffect(() => {
    ...
    if (contextProp?.permissions) {
      setmodulepermission(
        contextProp?.permissions?.role_permissions?.society || {}
      );
    }
  }, [contextProp]);
  ...
  const [socityCode, setSocityCode] = useState();
  ...
  //incharge & viewer modal states --> row expand
  ...
  const buildValidationSchema = (modules) => {
    ...
    return yup.object().shape({
      code: yup.string(),
      name: yup.string().trim().required("Society Name is required"),
  ...
  // const initial = {
  //   code: "", name: "", address: "", place: "", socity_id: "", community_id: null, country_id: null, state_id: null, region_id: null, district_id: null, incharge_user_id: null, viewr_user_id: null,
  //   // Add dynamic fields
  ...
  const createViewDynamicModules = (backendData) => {
    const modules = backendData.map((item, index) => {
      ...
      const ddmUsers = item?.sfp_user?.find(
        (user) => user?.role_id === RoleIds?.DDMRoleId
      );
      const viewers = item?.sfp_user?.filter(
  ...
  const getSocietyDetails = (id, type) => {
    ...
    getAPI("/config/society/" + id)
      .then((res) => {
        // console.log("society Data:", res?.data);

        if (res?.data?.status) {
          if (type === "expanded") {
            setExpandedData(res?.data?.data?.sfp);
            // setFetchedIds((prev) => new Set(prev).add(id));
          } else {
            setValue("id", res?.data?.data?.id);
            setValue("name", res?.data?.data?.name || "");
            setValue("code", res?.data?.data?.acme_code || "");
            ...
            // const ddmUsers = res?.data?.data?.society_user.filter(user => user.role_id === RoleIds.DDMRoleId);
            const viewerUsers = res?.data?.data?.society_user.filter(
              (user) => user.role_id === RoleIds.ViewerRoleId
            );
            const ddmUsers = res?.data?.data?.society_user.find(
              (user) => user.role_id === RoleIds.DDMRoleId
            );
            // const viewerUsers = res?.data?.data?.society_user.find(user => user.role_id === RoleIds.ViewerRoleId);
            ...
            createViewDynamicModules(res?.data?.data?.sfp);

            // res?.data?.data?.sfp?.map((item, index) => {
            //   const ddmUsers = item?.sfp_user?.find((user) => user?.role_id === RoleIds?.DDMRoleId);
            //   const viewers = item?.sfp_user?.filter((user) => user?.role_id === RoleIds?.ViewerRoleId);
            ...
            res?.data?.data?.sfp?.map((item, index) => {
              ...
              const viewers = item?.sfp_user?.find(
                (user) => user?.role_id === RoleIds.ViewerRoleId
              );
  ...
  const handleView = (id) => {
    ...
    getAPI("audit/society/" + id).then((res) => {
      if (res?.data?.status) {
  ...
  const getPortfolio = () => {
    getAPI(
      "/config/portfolio?skip=0&limit=25&type=Non%20Financial&search=Society"
    )
      .then((res) => {
        if (res?.data?.status) {
  ...
  const getSocietyList = useCallback(
    (search) => {
      setLoading(true);
      ...
      if (
        ...
        getAPI(
          `/config/society?skip=${pagination?.skip}&limit=${pagination?.limit}&province_id=` +
            filterProvince +
            "&search=" +
            search
        )
        ...
        getAPI(
          `/config/society?skip=${pagination?.skip}&limit=${pagination?.limit}&search=` +
            search
        )
          .then((res) => {
            if (res?.data?.status) {
        ...
        getAPI(
          `/config/society?skip=${pagination?.skip}&limit=${pagination?.limit}`
        )
          .then((res) => {
            if (res?.data?.status) {
              setSocietyList(res?.data?.data);
  ...
  const handleRowEditClick = (rowData) => {
    reset(rowData);
    setValue("country_id", {
    ...
    setValue("state_id", {
      value: rowData?.state?.id,
      label: rowData?.state?.name,
    });
    setValue("district_id", {
      value: rowData?.district?.id,
      label: rowData?.district?.name,
    });
    setValue("region_id", {
      value: rowData?.region?.id,
      label: rowData?.region?.name,
    });
    setValue("community_id", {
      value: rowData?.community?.id,
      label: rowData?.community?.name,
    });
    setSelectedData(rowData);
  };

  const onSubmit = (data) => {
    // Merge both `viewr_user_id` and `incharge_user_id` into `society_user`
    // Merge both `viewr_user_id` and `incharge_user_id` into `society_user`
    ...
    const societyUsers = combinedCommityUsers.map((user) => {
      return {
        user_id: user?.value,
        role_id: user?.roleId,
      };
    ...
    let apiData = {
      community_id: Number(data?.community_id?.value),
      // "acme_code": data.acme_code,
      // "code": data?.code,
      name: data?.name,
      place: data?.place,
      address: data?.address,
      country_id: Number(data?.country_id?.value),
      state_id: Number(data?.state_id?.value),
      region_id: Number(data?.region_id?.value),
      district_id: Number(data?.district_id?.value),
      society_user: societyUsers || [],
      sfp: transformedData || [],
    };

    const method = !isEdit ? "POST" : "PUT";
    const url = !isEdit ? "/config/society/" : `/config/society/${data?.id}/`;
  ...
  const onSubmit1 = (data) => {
    ...
    const mapping = "POST";
    addUpdateAPI(mapping, "config/society/import", formData).then((res) => {
      if (res?.data?.status) {
        getSocietyList();
        setLoading(false);
        //success
  ...
  const downloadExcel = () => {
    fetch(API_BASE_URL + "config/society/sample-excel", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = "Society_Form.xlsx"; // Change the file name if needed
  ...
  const downloadCommunityExcel = () => {
    fetch(API_BASE_URL + "config/society/export", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((response) => {
        response.blob().then((blob) => {
          ...
          const a = document.createElement("a");
          a.href = url;
          a.download = "Society_List.xlsx"; // Change the file name if needed
          document.body.appendChild(a);
          a.click();
          a.remove();
        });
  ...
  const columns = [
          ...
          <div key={row.id} className="d-flex align-items-center gap-2 w-100">
            ...
            {modulepermission?.edit && (
              <button
                ...
                onClick={() => {
                  const oldIncharge = row?.society_user?.find(
                    (item) => item?.role?.name === "DDM"
                  );
                  setEditInchargeData({
                    row: {
    ...
    //   width: "250px",
    //   cell: (row, index) => {
    //     const viewer = row.society_user?.find(
    ...
    //             onClick={() => {
    //               const oldViewer = row.society_user?.find(
    //                 (item) => item?.role?.name === "Viewer"
    //               );
    ...
    },
    {
      ...
      center: true,
      cell: (row) => {
        const userRoles = row?.society_user
                ...
                <>
                  {/* <div className="form_col ml-1">
                      <span className="custum-group-table" >
                        <button type="button" className="btn  btn-sm text-info" title='Manage files' onClick={() => mainManageFiles(row, navigate, portfolioIds, 'society')}  >
          ...
          <tbody>
            {rowLoading ? (
              ...
              expandedData?.map((portfolio, index) => {
                const hasAccess = portfolio?.sfp_user?.some(
                  (user) =>
  ...
  const handleInchargeChange = (selectedOption) => {
    dynamicModules.forEach((item) => {
      ...
      // setValue(`portfolio_${item.portfolio_id}.incharge_user_id`, selectedOption);
    });
  };
    ...
    <>
      <div>
        <div className="d-flex justify-content-between p-2 flex-wrap bg-white">
          <div className="p-2 col-lg-6 col-12">
          ...
          {/* <div className='d-flex justify-content-end col-lg-6  p-2 col-12 flex-wrap gap-1'>
            <div className="me-2 d-flex align-items-center ">
              <button className='btn bnt-sm adminsearch-icon'>
                <i className="fa fa-search " aria-hidden="true"></i>
              </button>
              <input type="text" className="form-control adminsearch" value={search} placeholder="Search by Code, Name, Place, District" title="Search" onChange={(e) => { getSocietyList(e.target.value), setSearch(e.target.value) }} />
            </div>
            {

              currentUser?.role?.name === "Admin" || currentUser?.role?.name === "Super Admin" ?
                <>
                  <button className='btn btn-sm px-4 adminBtn' title='Add' data-bs-toggle="modal" data-bs-target="#addModel" onClick={() => {
                    setIsEdit(false),
                      reset(initial);
                  }} >Add </button>
                  <button className='btn btn-sm px-4   btn-success' data-bs-toggle="modal" data-bs-target="#fileImportModal" title='Import' onClick={() => {
          ...
          <div className="d-flex justify-content-end col-lg-6 p-2 col-12 flex-wrap gap-2 align-items-end">
            {/* Search Input */}
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">
                {isEdit ? "Edit Society" : "Add Society"}{" "}
              </h5>
              <button
                type="button"
                ...
                aria-label="Close"
                onClick={() => {
                  reset(initial); // Reset form values using react-hook-form
                  setIsEdit(false);
                }}
              ></button>
            </div>
              ...
              <div className="modal-body">
                <div className="row ms-1">
                  ...
                  <div className="col-md-3 mb-1">
                    <label className="form-label">
                      Society Name (As per certificate){" "}
                      <span className="text-danger">*</span>
                    </label>
                    <input
                      type="text"
                ...
                <div className="modal-footer">
                  ...
                  <button type="submit" className="btn btn-sm adminBtn">
                    Submit
                  </button>
                </div>
              </div>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
                  ...
                  <label className="form-label">Rector</label>
                  {
                    ...
                    watch("viewr_user_id") ? (
                      ...
                      <p className="ms-2 fw-bold mb-0">
                        <span className="badge text-bg-secondary">No Rector</span>
                      </p>
                    )
                  }
                </div>
              ...
              {viewsDynamicModules.map((item, index) => (
                // Display each portfolio's details
                // console.log("finadasdf", item),
                    ...
                    {Array.isArray(item?.viewer) && item.viewer.length > 0 ? (
                      <div className="ms-2 fw-bold mb-0 d-flex flex-wrap gap-1">
                        {item.viewer.map((viewer, idx) => (
                          <p key={idx} className=" ms-2 fw-bold mb-0">
                            {viewer.name}
                          </p>
                        ))}
                      </div>
      ...
      >
        <div className="modal-dialog modal-md">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Import Society</h5>
              <button
                type="button"
                className="btn-sm btn-close"
                data-bs-dismiss="modal"
      ...
      >
        <div className="modal-dialog modal-dialog-centered"
          ...
          <div className="modal-content" style={{ height: "350px", width: "1500px" }}>
            <div className="modal-header">
              ...
              <div className="flex-fill">
                ...
                <Select
                  value={editViewerData.newViewer ||
                    (editViewerData.oldViewer ?
                      {
                        value: editViewerData.oldViewer.user_id,
                        label: editViewerData.oldViewer.user.name,
                        roleId: RoleIds.ViewerRoleId
                      }
                      : null
                    )
          ...
          >
            ...
            <div className="modal-body d-flex justify-content-between align-items-center gap-2">
              ...
              <div className="flex-fill">
                ...
                <Select
                  ...
                  //   (editViewerData.oldViewer ? {
                  //     value: editViewerData.oldViewer.value,
                  //     label: editViewerData.oldViewer.label || 'Unknown',
                  //     roleId: RoleIds.ViewerRoleId
                  //   } : null)
                  // }
                  options={vieweruseroptions?.map((data) => ({
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                className="btn btn-success"
                onClick={handleSaveViewer}
                disabled={!editViewerData.newViewer}
              >
                Save
              </button>
            </div>
          ...
          >
            ...
            <div className="modal-body d-flex justify-content-between align-items-center gap-2">
              ...
              <div className="flex-fill">
                ...
                <Select
                  value={
                    editInchargeData?.newIncharge
                    // ||
                    // (editInchargeData?.oldIncharge ?
                    //   {
                    //     value: editInchargeData.oldIncharge.user_id,
                    //     label: editInchargeData.oldIncharge.user.name,
                    //     roleId: RoleIds.DDMRoleId
                    //   }
                    //   : null
                    // )
