...
function Department() {
  ...
  // const modulepermission = permissions?.role_permissions?.departments;
  ...
  useEffect(() => {
    ...
    if (contextProp?.permissions) {
      setmodulepermission(
        contextProp?.permissions?.role_permissions?.departments || {}
      );
    }
  }, [contextProp]);
  ...
  const getDynamicInitial = (modules) => {
    const dyn = {};
    modules.forEach((item) => {
      dyn[`portfolio_${item.portfolio_id}`] = {
        [item.type]: null,
        [item.key]: "",
        [item.key1]: "",
  ...
  const initial = {
    ...
    // ...getDynamicInitial(dynamicModules), // Add dynamic fields
  };
  ...
  const getPortfolio = () => {
    getAPI(
      "/config/portfolio?skip=0&limit=25&type=Non%20Financial&search=Department"
    )
      .then((res) => {
        if (res?.data?.status) {
                  ...
                  {/* <div className="form_col ml-1">
                      <span className="custum-group-table" >
                        <button type="button" className="btn  btn-sm text-info" title='Manage files'
                          //  onClick={() => { contextProp.setNavState({ ...row, module: 'Department' }); navigate('/nonfinancial/department') }} 
                          onClick={() => mainManageFiles(row, navigate, portfolioIds, 'department')}
                        >
  ...
  const onSubmit1 = (data) => {
    ...
    const mapping = "POST";
    addUpdateAPI(mapping, "config/departments/import", formData).then((res) => {
      if (res?.data?.status) {
        getPortfolios();
        getUserOptions();
        setLoading(false);
  ...
  const downloadExcel = () => {
    fetch(API_BASE_URL + "config/entities/sample-export?portfolio_id=8", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = "Department_Form.xlsx"; // Change the file name if needed
  ...
  const downloadCommunityExcel = () => {
    fetch(API_BASE_URL + "config/entity-portfolio/export?portfolio_id=8", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      ...
      .then((response) => {
        response.blob().then((blob) => {
          ...
          const a = document.createElement("a");
          a.href = url;
          a.download = "Department_List.xlsx"; // Change the file name if needed
          document.body.appendChild(a);
          a.click();
          a.remove();
        });
    ...
    <>
      <div>
        <div className="d-flex justify-content-between p-2 flex-wrap bg-white">
          <div className="p-2 col-lg-6 col-12">
            {/* <h6 className="fw-bold mb-0">Department</h6> */}
          ...
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button
                ...
                aria-label="Close"
                onClick={() => {
                  reset(); // Reset form values using react-hook-form
                  setIsEdit(false);
                }}
              ></button>
            </div>
      ...
      >
        <div className="modal-dialog modal-md">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Import Department</h5>
              <button
                type="button"
                className="btn-sm btn-close"
                data-bs-dismiss="modal"
