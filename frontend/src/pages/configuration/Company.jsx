...
function Company() {
  ...
  // const modulepermission = permissions?.role_permissions?.companies;
  ...
  useEffect(() => {
    ...
    if (contextProp?.permissions) {
      setmodulepermission(
        contextProp?.permissions?.role_permissions?.companies || {}
      );
    }
  }, [contextProp]);
    ...
    getAPI("/config/community?skip=0&limit=0")
      .then((res) => {
        if (res?.data.status) {
          ...
          setLoading(false); // Handle false status from API
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const getPortfolio = () => {
    getAPI(
      "/config/portfolio?skip=0&limit=25&type=Non%20Financial&search=Companies"
    )
      .then((res) => {
        if (res?.data?.status) {
  ...
  const getCompanyList = useCallback(
    (search) => {
      ...
      if (portfolioIds != undefined && search != undefined) {
        getAPI(
          `/config/entity?skip=${pagination.skip}&limit=${pagination.limit}&portfolio_id=${portfolioIds}&search=${search}`
        )
          .then((res) => {
            if (res?.data.status) {
      ...
      .then((res) => {
        if (res?.data?.status) {
          if (type === "expanded") {
            ...
            // setValue("socity_id", { "value": res?.data?.data?.society?.id, "label": res?.data?.data?.society?.name })
            // setValue("socity_id", { "value": res?.data?.data?.society?.id, "label": res?.data?.data?.society?.name })
                  ...
                  {/* <div className="form_col ml-1">
                      <span className="custum-group-table" >
                        <button type="button" className="btn  btn-sm text-info" title='Manage files'
                          // onClick={() => { contextProp.setNavState({ ...row, module: 'School' }); navigate('/nonfinancial/company') }} 
                          onClick={() => mainManageFiles(row, navigate, portfolioIds, 'company')}
                        >
  ...
  const rowExpandView = (row) => {
    return (
      <div key={row?.id} className="p-2 ps-3 pt-3 subtable d-flex ">
  ...
  const onSubmit1 = (data) => {
    ...
    const mapping = "POST";
    addUpdateAPI(mapping, "config/companies/import", formData).then((res) => {
      if (res?.data?.status) {
        getUserOptions();
        getCompanyList();
        setLoading(false);
  ...
  const downloadExcel = () => {
    fetch(API_BASE_URL + "config/entities/sample-export?portfolio_id=10", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
  ...
  const downloadCommunityExcel = () => {
    fetch(API_BASE_URL + "config/entity-portfolio/export?portfolio_id=10", {
      method: "GET",
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          ...
          <div className="d-flex justify-content-end col-lg-6 p-2 col-12 flex-wrap gap-2 align-items-end">
            <div className="d-flex flex-grow-1" style={{ maxWidth: "300px" }}>
              ...
              <input
                ...
                title="Search by name, place, address"
                value={search}
                onChange={(e) => {
                  getCompanyList(e.target.value), setSearch(e.target.value);
                }}
              />
            </div>
            {currentUser?.role?.name === "Admin" ||
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="modal-body">
                <div className="row ms-1">
                  ...
                  {/* {isEdit &&<div className="mb-3 col-md-3">
                    <label className="form-label">
                  ...
                  <div className="col-md-3 mb-1">
                    <label className="form-label">
                      Company Name (As per certificate){" "}
                      <span className="text-danger">*</span>
                    </label>
                    <input
                      type="text"
            ...
            <div className="modal-body">
              <div className="row ms-1 border-bottom border-secondary mb-0">
                ...
                <div className="col-md-3 mb-1">
                  <label className="form-label">Incharge</label>
                  <p className="ms-2 fw-bold">
                    {watch("incharge_user_id")?.label || "N/A"}
                  </p>
                </div>
